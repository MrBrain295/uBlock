...
unsigned_xpi_filepath = os.path.join(tmpdir.name,
                                     'uBlock0.firefox.unsigned.xpi')
...
release_info_url = 'https://api.github.com/repos/{0}/{1}/releases/tags/{2}'.format(
    github_owner, github_repo, tag_version)
headers = {
        ...
        for item in zipin.infolist():
            ...
            if item.filename == 'manifest.json':
                ...
                manifest['browser_specific_settings']['gecko'][
                    'update_url'] = 'https://raw.githubusercontent.com/{0}/{1}/master/dist/firefox/updates.json'.format(
...
with open(unsigned_xpi_filepath, 'rb') as f:
    ...
    headers = {
        'Authorization': get_jwt_auth(),
    }
    data = {'channel': 'unlisted'}
    files = {
        'upload': f,
    }
    signing_url = 'https://addons.mozilla.org/api/v4/addons/{0}/versions/{1}/'.format(
        extension_id, ext_version)
    print('Submitting package to be signed...')
    response = requests.put(signing_url,
                            headers=headers,
                            data=data,
                            files=files)
    if response.status_code != 202:
        print('Error: Creating new version failed -- server error {0}'.format(
            response.status_code))
        print(response.text)
        exit(1)
    ...
    f.close()
    print(
        'Waiting for AMO to process the request to sign the self-hosted xpi package...'
    )
    ...
    while True:
        ...
        if response.status_code != 200:
            print('Error: AMO signing failed -- server error {0}'.format(
                response.status_code))
            print(response.text)
            exit(1)
        ...
        if not signing_check_response['valid']:
            ...
            print(response.text)
            exit(1)
        if not signing_check_response['files'] or len(
                signing_check_response['files']) == 0:
            continue
        ...
        print('Downloading signed self-hosted xpi package from {0}...'.format(
            download_url))
        response = requests.get(download_url, headers=headers)
        if response.status_code != 200:
            print('Error: Download signed package failed -- server error {0}'.
...
with open(signed_xpi_filepath, 'rb') as f:
    url = release_info['upload_url'].replace('{?name,label}',
                                             '?name=' + signed_xpi_filename)
    headers = {
        'Authorization': github_auth,
    ...
    if response.status_code != 201:
        print(
            'Error: Upload signed package failed -- server error: {0}'.format(
                response.status_code))
        exit(1)
...
if response.status_code != 204:
    print('Error: Deletion of raw package failed -- server error: {0}'.format(
        response.status_code))
...
updates_json_filepath = os.path.join(projdir, 'dist', 'firefox',
                                     'updates.json')
with open(updates_json_filepath) as f:
    updates_json = json.load(f)
    f.close()
    previous_version = updates_json['addons'][extension_id]['updates'][0][
        'version']
    if LooseVersion(ext_version) > LooseVersion(previous_version):
        with open(
                os.path.join(projdir, 'dist', 'firefox',
                             'updates.template.json')) as f:
            template_json = Template(f.read())
            f.close()
            updates_json = template_json.substitute(ext_version=ext_version,
        ...
        r = subprocess.run(['git', 'status', '-s', updates_json_filepath],
        ...
        if len(rout) >= 2 and rout[0] == 'M':
            subprocess.run([
                'git', 'commit', '-m', 'Make Firefox dev build auto-update',
                updates_json_filepath
            ])
